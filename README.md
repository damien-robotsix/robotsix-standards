...
| [CLI documentation validation](docs/cli-documentation-validation.md) | CLI command examples in public documentation must match the real argument parser and be validated by subprocess tests, so a documented invocation that no longer parses fails CI instead of shipping an ImportError to users. |
