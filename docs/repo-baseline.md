...
[changelog & releases](#changelog-releases). A release publishes nothing to<!-- markdownlint-disable-line MD051 -->
any package index.)
...
- **Security:** Semgrep (SAST), zizmor workflow audit, GitHub secret scanning + push protection
  ...
  security workflow), a dependency CVE audit (`uv audit --preview-features audit-command` at a pinned
  uv version in the CI gate — preview-only, see
  [security posture](security-posture.md#6-sbom-vulnerability-audit) — plus `pip-audit` in the security
...
**Rule:** Every job in every GitHub Actions workflow MUST declare an
explicit `timeout-minutes` ceiling.  The GitHub default is 6 hours
(360 minutes); without an explicit ceiling a hung or runaway job
consumes that full window — wasting CI minutes on every subsequent
push until someone notices and kills it.
...
Python-specific timeout defaults (lint: 5, type-check: 10, test: 15)
are listed in [Python CI workflow](python-ci-workflow.md#job-timeout-minutes).
